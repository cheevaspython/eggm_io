...
Используется подход:
- SimpleFakeLLM (свой Mock класс, не unittest.mock)
- Реальные промпты
- Фикстуры для State
"""
...
from source.tests.unit.fake_llm import SimpleFakeStructuredLLM
...
class TestSupervisorServiceRouting:
    ...
    async def test_routes_to_reminder_agent(
        ...
        fake_llm = SimpleFakeStructuredLLM(
            responses=[
                SupervisorDecision(
    ...
    async def test_routes_to_confirm_agent(
        self,
        supervisor_prompts: SupervisorPrompts,
    ):
        """Супервизор маршрутизирует на confirm_agent."""
        state = State(
            telegram_user_id=uuid4(),
            telegram_chat_id=123456,
            crm_user_id=100,
...
class TestSupervisorServiceStateHandling:
    ...
    async def test_preserves_existing_task_type(
        ...
        # LLM не должен вызываться, но создадим на всякий случай
        fake_llm = SimpleFakeStructuredLLM(
    ...
    async def test_preserves_other_state_fields(
        ...
        assert result_state.telegram_user_id == state.telegram_user_id
        assert result_state.telegram_chat_id == state.telegram_chat_id
        assert result_state.user_input == state.user_input
...
class TestSupervisorServiceErrorHandling:
    ...
    async def test_fallback_on_llm_error(
        ...
        # Создаем LLM который выбрасывает ошибку
        class ErrorLLM(SimpleFakeStructuredLLM):
    ...
    async def test_handles_dict_response(
        ...
        # SimpleFakeStructuredLLM который возвращает dict
        fake_llm = SimpleFakeStructuredLLM(
            responses=[
                {
                    "task_type": TaskTypeChoises.edit_deal_agent,
